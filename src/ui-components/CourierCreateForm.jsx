...
export default function CourierCreateForm(props) {
  ...
  const initialValues = {
    ...
    lat: "",
    lng: "",
    transporationMode: undefined,
  };
  const [name, setName] = React.useState(initialValues.name);
  const [sub, setSub] = React.useState(initialValues.sub);
  const [lat, setLat] = React.useState(initialValues.lat);
  const [lng, setLng] = React.useState(initialValues.lng);
  const [transporationMode, setTransporationMode] = React.useState(
    initialValues.transporationMode
  );
  ...
  const resetStateValues = () => {
    ...
    setTransporationMode(initialValues.transporationMode);
    setErrors({});
  };
  const validations = {
    name: [{ type: "Required" }],
        ...
        onChange={(e) => {
          ...
          if (onChange) {
            const modelFields = {
              ...
              lat,
              lng,
              transporationMode: value,
            };
            const result = onChange(modelFields);
            value = result?.transporationMode ?? value;
          }
          if (errors.transporationMode?.hasError) {
            runValidationTasks("transporationMode", value);
          }
          setTransporationMode(value);
        }}
        onBlur={() =>
          runValidationTasks("transporationMode", transporationMode)
        }
        errorMessage={errors.transporationMode?.errorMessage}
        hasError={errors.transporationMode?.hasError}
        {...getOverrideProps(overrides, "transporationMode")}
      >
        <option
          children="Driving"
          value="DRIVING"
          {...getOverrideProps(overrides, "transporationModeoption0")}
        ></option>
        <option
          children="Bicycling"
          value="BICYCLING"
          {...getOverrideProps(overrides, "transporationModeoption1")}
        ></option>
      </SelectField>
      <Flex
